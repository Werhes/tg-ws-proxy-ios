...
> **Фоновый режим:** на главной странице (вкладка «Прокси», внизу) можно выбрать способ работы в фоне, пока прокси запущен: **Выключено**, **Тихий звук** (бесшумный аудио-цикл), **Геолокация** (фоновые обновления геопозиции), **VPN** (активация VPN-туннеля), **Микрофон** (бесшумная запись) или **Камера** (работающая камера). Так процесс остаётся активным, когда приложение свёрнуто или заблокирован экран. Эти методы подходят для установки через AltStore/Sideloadly. При выборе «Микрофон»/«Камера» система запросит соответствующие разрешения. Режим **VPN** требует entitlement Personal VPN и настроенной конфигурации. Для публикации в App Store корректным решением является Network Extension (VPN) — потребуются соответствующие entitlements и платный аккаунт разработчика.
...
- `create_release` — чекбокс **«Создать GitHub Release»**: при включении создаётся релиз с `.ipa`. Если выключен — сборка и артефакты делаются без релиза.

Эти значения передаются в GitHub Release (тег, название, описание). Релиз при ручном запуске создаётся только при включённом `create_release`.
