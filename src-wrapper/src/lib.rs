...
/// Machine-parseable stats: "active,bytesUp,bytesDown,wsConnections".
#[no_mangle]
pub extern "C" fn GetRawStats() -> *mut c_char {
    let s = format!(
        "{},{},{},{}",
        STATS.connections_active.load(Ordering::Relaxed),
        STATS.bytes_up.load(Ordering::Relaxed),
        STATS.bytes_down.load(Ordering::Relaxed),
        STATS.connections_ws.load(Ordering::Relaxed)
    );
    CString::new(s).unwrap_or_default().into_raw()
}
...
#[no_mangle]
pub extern "C" fn GetCfProxyDomains() -> *mut c_char {
    if CFPROXY.read().domains.is_empty() {
        cfproxy::init_cfproxy_domains();
    }
    let domains = CFPROXY.read().domains.join("\n");
    CString::new(domains).unwrap_or_default().into_raw()
}

#[no_mangle]
pub extern "C" fn RefreshCfProxyDomains() -> c_int {
    if runtime().block_on(cfproxy::try_refresh_cfproxy_domains()) {
        1
    } else {
        0
    }
